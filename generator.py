...
class UNet(nn.Module):
    """
    Minimal U-Net Generator.
    Input/Output: (B, 1, H, W) in [-1, 1]

    Encoder: Conv stride-2 (halve size each step)
    Decoder: ConvTranspose stride-2 (double size) + skip connection
    Output:  Conv2d + Tanh  (NO extra upsample — size stays the same)
    """
    ...
    def __init__(self, base=32):
        ...
        # Decoder (each doubles H,W; input = prev_out + skip)
        ...
        # Output: plain Conv2d — does NOT change spatial size
        self.out = nn.Sequential(
            nn.Conv2d(b + 1, 1, 3, padding=1),   # d4 + original input
            nn.Tanh(),
        )
    ...
    def forward(self, x):
        ...
        # Final output (align to input, concat, conv)
        d4 = self._pad(d4, x)
